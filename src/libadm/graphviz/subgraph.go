...
func (s *subgraph) Init(title string, properties NodeProperties, tabcount int) error {
	if title == "" { return errors.New("graphviz subgraph title cannot be empty")}
	s.label = title
	s.properties = properties
	s.tabSpaceCount = tabcount
	if s.Nodes == nil { s.Nodes = make(map[string]string) }
	if s.Edges == nil { s.Edges = make(map[string][]spec) }
